...
def main():
    ...
    sta = parser_val.sta
    cmt = parser_val.cmt
    cmt_type='*MXZ.sem.sac'
    syn = parser_val.syn
    res_dir = '{cwd}/{res_dir}'.format(cwd=os.getcwd(), res_dir=parser_val.res_dir)
    cwd = os.getcwd()
    make_res_dir(res_dir)
    station_list = parser_val.sta.split(",")
    print(station_list)
    cat = obspy.read_events(cmt + '/CMTSOLUTION')
    cat[0].origins[0].time.year += YEAR

    # I dont know what this stuff is or where it should go
    ...
    for stations in station_list:
        files = glob.glob('{cmt}/{net}.{sta}.{cmt_type}'.format(cmt=cmt, net=net, sta=stations,cmt_type=cmt_type))  # SYNTHETICS
        for current_file in files:
            current_file = current_file.replace('MXZ', 'MX*')
            try:
                st = read(syn + str(cat[0].origins[0].time.year) + '/C' + str(cat[0].origins[0].time.year) +
                              str(cat[0].origins[0].time.month).zfill(2) + str(cat[0].origins[0].time.day).zfill(2) + '*/'
                              + '*' + current_file.split('.')[1] + '*.sac')
            except:
                continue
            st.integrate()
            st.integrate()
    ...
    # ### ADDING IN SPECFC data
        ...
        # rotate and remove response
            ...
            for idx, chan in enumerate(['Z', 'N', 'E']):
                ...
                if idx == 1:
                    filename= get_file_name(res_dir, st_data)
...
def getargs():
    ...
    parser.add_argument('-cmt', type=str, action="store",
                        dest="cmt", required=True,
                        help="CMT OUTPUT directory location Example: " +
                        "/EXAMPLES/global_asl/OUTPUT_FILES/CMTSOLUTION")
